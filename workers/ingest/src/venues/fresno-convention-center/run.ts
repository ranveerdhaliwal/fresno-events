...
async function parseConventionListing(
  _env: IngestEnv,
  ...
  return parseConventionListingHtml(html, venueConfig, new Date());
}

export const run = (env: IngestEnv, ctx: VenueRunContext): Promise<VenueRunResult> =>
  runHtmlParseVenue(env, config, ctx, parseConventionListing);
