import type { Event, EventListItem, ImageAsset, Venue } from "@fresno-events/shared";
...
export function getMockEventList(): EventListItem[] {
  const now = new Date();
...
export function getMockEventBySlug(slug: string): EventListItem | null {
  return getMockEventList().find((item) => item.event.slug === slug) ?? null;
}

function listItem(input: { event: Event; venue: Venue; heroImage: ImageAsset }): EventListItem {
  return { event: input.event, venue: input.venue, heroImage: input.heroImage };
}
