...
| `pnpm ingest:promote-apis` | **Real** APIs + gobulldogs (run preflight separately if you want) |
| `pnpm ingest:promote-crawl` | **Real** ai-crawl only |
...
5. After schema change: `pnpm db:reset` locally, or apply migration on cloud; re-run `pnpm ingest:enrich` to backfill `suggested_priority` on existing pending rows
